    ...
    function crearMallaInteractiva() {
      ...
      for (const [semestre, ramos] of Object.entries(malla)) {
        ...
        ramos.forEach(ramo => {
          ...
          info.className = "prerrequisitos";
          info.textContent = requisitos.length ? `Prerrequisitos: ${requisitos.join(", ")}` : "Sin prerrequisitos";
          ...
          divRamo.addEventListener("click", () => {
            if (requisitos.every(req => aprobados.has(req))) {
              ...
              alert(`Ramo aprobado: ${ramo.nombre}`);
            } else {
              alert("Aún no cumples con los prerrequisitos");
            }
          });
